	...
	var ProgressBars = Ractive.extend({
		...
		events:{
			...
			tap: function( node, fire ) {
				var longpressDuration = 150, tapHandler, startHandler, cancelHandler, timeout;
				startHandler = function ( event ){				
					node.addEventListener( 'mouseup', tapHandler );
					node.addEventListener( 'touchend', tapHandler );
					window.addEventListener( 'mousemove', cancelHandler );
					window.addEventListener( 'touchmove', cancelHandler );
				};
				cancelHandler = function(event){
				   	clearTimeout( timeout );
				   	window.removeEventListener( 'mousemove', cancelHandler );	
				   	window.removeEventListener( 'touchmove', cancelHandler );	
